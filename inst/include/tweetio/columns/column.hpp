...
constexpr auto val_getter(const std::string_view pointer, F custom_getter) {
...
constexpr auto custom_col_spec(const std::string_view col_name, Ts... targets) {
      ...
      if (auto [array2, array2_error] =
        ...
        for (auto array3 : array2) {
          ...
          for (auto element :
            ...
            out[i + j] = element.template get<inner_T>().take_value();
            i += 4;
          }
          j++;
        }
...
template <typename T, typename inner_T>
constexpr auto campaign_tags_col_spec(const std::string_view col_name,
                                      const std::string_view pointer) {
  auto init = [pointer](auto x) -> T {
    if (auto [array, error] =
