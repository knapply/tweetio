...
template <typename stream_T> inline size_t count_lines(const std::string& file_path) {
  stream_T in_file;
...
inline constexpr auto count_digits(const int x) -> int {
  if (x < 0) {
...
inline void finalize_df(Rcpp::List& x, const vec_chr& col_names, const int& n_rows) {
