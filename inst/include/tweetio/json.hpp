...
// inline void get_int2(int& init, const rapidjson::Value& x) {
//   if (x.IsInt()) {
//     init = x.GetInt();
//   }
//   // return x.IsInt() ? x.GetInt() : NA_INTEGER;
...
inline vec_chr map_entities(const rapidjson::Value& x,
  ...
  const R_xlen_t ext_n = x["extended_tweet"]["entities"][entity.c_str()].Size();
  const R_xlen_t reg_n = x["entities"][entity.c_str()].Size();
...
inline vec_chr get_nested_meta_results(const rapidjson::Value& x,
  ...
  const R_xlen_t out_n = results.Size();
  if (out_n == 0) {
    return default_val;
  }
